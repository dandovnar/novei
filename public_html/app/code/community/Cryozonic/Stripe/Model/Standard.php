...
class Cryozonic_Stripe_Model_Standard extends Mage_Payment_Model_Method_Abstract {
    ...
    const MODULE_URL = "https://stripe.com/docs/magento/cryozonic";
    ...
    public function __construct()
    {
        ...
        $this->paymentIntent = Mage::getSingleton("cryozonic_stripe/paymentIntent");
        $this->saveCards = $this->store->getConfig('payment/cryozonic_stripe/ccsave');
    ...
    public function addOn($name, $version, $url = null)
    {
        ...
        $name = Cryozonic_Stripe_Model_Standard::MODULE_NAME;
        $version = Cryozonic_Stripe_Model_Standard::MODULE_VERSION;
        $url = Cryozonic_Stripe_Model_Standard::MODULE_URL;
    ...
    {
        if (!Mage::helper("cryozonic_stripe")->isConfigured())
            return;
    ...
    public function createCharge(Varien_Object $payment, $capture, $forceUseSavedCard = false)
    {
        ...
        try {
            ...
            if (strpos($token, "pm_") === 0)
            {
                ...
                if ($capture)
                    $this->paymentIntent->capture = Cryozonic_Stripe_Model_PaymentIntent::CAPTURE_METHOD_AUTOMATIC;
                else
                    $this->paymentIntent->capture = Cryozonic_Stripe_Model_PaymentIntent::CAPTURE_METHOD_MANUAL;
            ...
            if (!$charge->captured && $this->getStore()->getConfig('payment/cryozonic_stripe/automatic_invoicing'))
            {
    ...
    public function cancel(Varien_Object $payment, $amount = null)
    {
        ...
        try {
            ...
            if (strpos($transactionId, 'pi_') === 0)
            {
                ...
                if ($pi->status == Cryozonic_Stripe_Model_PaymentIntent::AUTHORIZED)
                {
                    $pi->cancel();
                    return $this;
                }
    ...
    {
        if (!Mage::helper("cryozonic_stripe")->isEnabled())
            return false;
    ...
    {
        $info = array(
            "Active: " . (int)$this->store->getConfig('payment/cryozonic_stripe/active'),
            "Apple Pay: " . (int)$this->store->getConfig('payment/cryozonic_stripe/apple_pay_checkout'),
            "Location: " . (int)$this->store->getConfig('payment/cryozonic_stripe/apple_pay_location'),
            "Invoice: " . (int)$this->store->getConfig('payment/cryozonic_stripe/automatic_invoicing'),
            "Action: " . (int)$this->store->getConfig('payment/cryozonic_stripe/payment_action'),
            "Countries: " . (int)$this->store->getConfig('payment/cryozonic_stripe/allowspecific')
        );
