...
class Cryozonic_Stripe_Model_PaymentIntent
{
    ...
    protected function getParamsFrom($quote, $payment = null)
    {
        ...
        $this->params['automatic_payment_methods'] = ['enabled' => 'true']; // For now
  
        $this->setCrossBorderClassification($quote);

        $statementDescriptor = Mage::getStoreConfig('payment/cryozonic_stripe/statement_descriptor');
    ...
    {
        $isModuleEnabled = Mage::helper("cryozonic_stripe")->isEnabled();
    ...
    public function isSuccessfulStatus($paymentIntent = null)
    {
        ...
        return ($paymentIntent->status == Cryozonic_Stripe_Model_PaymentIntent::CAPTURED ||
            $paymentIntent->status == Cryozonic_Stripe_Model_PaymentIntent::AUTHORIZED);
    }
    ...
    public function getCaptureMethod()
    {
        ...
        if ($this->helper->isAuthorizeOnly())
            return Cryozonic_Stripe_Model_PaymentIntent::CAPTURE_METHOD_MANUAL;

        return Cryozonic_Stripe_Model_PaymentIntent::CAPTURE_METHOD_AUTOMATIC;
    }
    ...
    public function redirectToMultiShippingAuthorizationPage($payment, $paymentIntentId)
    {
        ...
        $session->setAuthorizationRedirect("cryozonic_stripe/authorization_multishipping");
    ...
    public function confirmAndAssociateWithOrder($order, $payment)
    {
        ...
        if (!$this->isSuccessfulStatus())
        {
            ...
            {
                $confirmParams = [];
                if ($this->helper->isAdmin() && Mage::getStoreConfig('payment/cryozonic_stripe/moto_exemptions'))
    ...
    public function processAuthenticatedOrder($order, $paymentIntent)
    {
        ...
        if (!$charge->captured && Mage::getStoreConfig('payment/cryozonic_stripe/automatic_invoicing'))
        {
            $payment->setIsTransactionPending(true);
