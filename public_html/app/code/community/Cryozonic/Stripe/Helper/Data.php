...
class Cryozonic_Stripe_Helper_Data extends Mage_Payment_Helper_Data
{
    ...
    {
        $store = $this->getStore();
        $enabled = $store->getConfig('payment/cryozonic_stripe/active');
        if (!$enabled)
            return false;
    ...
    public function initStripe()
    {
        ...
        \Stripe\Stripe::setApiVersion(Cryozonic_Stripe_Model_Standard::STRIPE_API);
        \Stripe\Stripe::setAppInfo(
            Cryozonic_Stripe_Model_Standard::MODULE_NAME,
            Cryozonic_Stripe_Model_Standard::MODULE_VERSION,
            Cryozonic_Stripe_Model_Standard::MODULE_URL,
            "pp_partner_DKh0AilNOHW2mb"
        );
    ...
    {
        return (Mage::getStoreConfig('payment/cryozonic_stripe/payment_action') == Mage_Payment_Model_Method_Abstract::ACTION_AUTHORIZE);
    }
    ...
    {
        return Cryozonic_Stripe_Model_Standard::MODULE_NAME . " v" . Cryozonic_Stripe_Model_Standard::MODULE_VERSION;
    }
    ...
    public function getStripeCustomer($id = null)
    {
        if (!empty($id))
        {
            $object = $this->getCachedObject('customer_' . $id);
            if ($object)
        ...
        try
        {
            ...
            $this->cacheObject($this->customer, 'customer_' . $id, 30);
            return $this->customer;
        }
        catch (\Exception $e)
        {
    ...
    public function getStripeAccountCountry()
    {
        ...
        if ($account['country'])
        {
            $cache->save($account['country'], $key, $tags = [], $lifetime = (356 * 24 * 60 * 60));
