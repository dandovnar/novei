...
var stripe = {
	...
	getStripeElementsOptions: function () {
		return {
			locale: 'auto',
			clientSecret: document.querySelector('.stripe-payment').id,
		};
	},
	initStripeElements: function () {
		...
		if (clientSecret) {
			...
			stripe.stripeJsV3.retrievePaymentIntent(clientSecret).then(function (result) {
				if (result.paymentIntent.status == 'requires_payment_method') {
					jQuery(message).html("Payment failed. Please try another payment method.");
				};
			});
		};
	},
	...
	initPaymentRequestButton: function () {
		...
		try {
			...
			var prButton = document.getElementById('payment-request-button');
		}
		catch (e) {
		...
		paymentRequest.canMakePayment().then(function (result) {
			if (result) {
				...
				prButton.style.display = 'inline-block';
				prButton.querySelector('.default').style.display = 'none';
				prButton.addEventListener('click', paymentRequest.show);
				$$('.stripexpress-logo').invoke('show');
				if (result.applePay) {
					prButton.querySelector('.default').style.display = 'none';
					prButton.querySelector('.applepay').style.display = 'inline';
				}
				else {
					prButton.querySelector('.default').style.display = 'inline';
				}
...
var affirm = document.getElementById('p_method_stripe_payments_affirm');
affirm.onclick = function (e) {
	jQuery.ajax({
		type: 'POST',
		url: '/stripe/verification/pi/',
		success: function (response) {
			response = jQuery.parseJSON(response);
		}
	});
	var button = document.getElementsByClassName('area-header');
	var affirm_button = "<a href=\"#\" class=\"affirm-payment\">Continue to Affirm</a>";
	var secret = document.querySelector('.stripe-payment').id;
		...
		oscPlaceOrder = function (element) {
			...
			if (validator.validate()) {
				if (affirm.checked) {
					...
					stripe.stripeJsV3.createPaymentMethod({
						type: 'affirm',
						data
					})
					*/
					var intent = stripe.stripeJsV3.retrievePaymentIntent(secret);
					console.log(intent);
					createStripeToken(function (err) {
						...
						stripe.stripeJsV3.confirmAffirmPayment(
							...
							{
								payment_method: { type: 'affirm', billing_details: stripe.getSourceOwner(), },
								return_url: 'https://www.sohomod.com/checkout/onepage/success',
							},
						);
						stripe.placeOrder();
					});
